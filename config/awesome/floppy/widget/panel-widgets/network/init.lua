...
local wlan_interface = 'wlp0s20u6'
local lan_interface = 'eno1'
...
local return_button = function()
	...
	local widget = wibox.widget {
		{
			id = 'icon',
			image = icons.symbolic.network.loading,
	...
	local update_disconnected = function()
		if conn_status == 'wireless' or conn_status == 'wired' then
			...
			if conn_status == 'wireless' then
				widget_icon_name = icons.symbolic.network.wifi_off
			elseif conn_status == 'wired' then
				widget_icon_name = icons.symbolic.network.wired_off
			end
	...
	local notify_no_access = function(strength)
		if conn_status == 'wireless' or conn_status == 'wired' then
			...
			if conn_status == 'wireless' then
				icon =  icons.symbolic.network.wifi_error
	...
	local update_no_access = function(strength)
		...
		if conn_status == 'wireless' then
			widget_icon_name = icons.symbolic.network.wifi_error
			-- widget_icon_name = 'wifi-strength-' .. tostring(strength) .. '-alert'
		elseif conn_status == 'wired' then
			widget_icon_name = icons.symbolic.network.wired_error
		end
		...
		awful.spawn.easy_async_with_shell(
			...
			function(stdout)
				...
				awful.spawn.easy_async_with_shell(
					...
					function(stdout)
						widget_icon_name = 'wifi_' .. wifi_strength_rounded
						...
						widget.icon:set_image(icons.symbolic.network[widget_icon_name])
					end
				)
			end
		)
	...
	local update_wired = function()
		...
		awful.spawn.easy_async_with_shell(
			check_internet_health,
			function(stdout)
				widget_icon_name = icons.symbolic.network.wired
